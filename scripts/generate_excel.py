...
This script receives data extracted by Claude (via visual analysis),
...
Example:
    python generate_excel.py --json extracted_data.json --output product_list.xlsx
...
def prepare_image_for_excel(image_path):
    """
    Prepare image for Excel embedding. Uses original image directly for maximum clarity.
    Returns the image path as-is (no compression/resize).
    """
    ...
    for row_idx, row_data in enumerate(data, 2):
        ...
        for col_idx, col_name in enumerate(fields, 1):
            ...
            if col_name.lower() in ["image", "图片", "图"] and value:
                ...
                prepared = prepare_image_for_excel(image_path)
                if prepared:
                    try:
                        xl_image = XLImage(prepared)
                        xl_image.width = 320
                        xl_image.height = 240
...
if __name__ == "__main__":
    ...
    try:
        ...
        if isinstance(input_data, dict):
            ...
            # If it's a list, use all keys from the first item as fields
            if not input_data:
